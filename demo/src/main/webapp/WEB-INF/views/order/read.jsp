								...
								<table class="table  text-center" id="ordertbl">
									...
									<c:set var="totalPrice" value="${totalPrice + orderList[status.index].pdt_price * orderList[status.index].odr_amount}"></c:set>
										<tr id="row">
											...
											<td class="col-md-1">
												<c:choose>
													<c:when test="${product.odr_status == 1}">입금완료</c:when>
													<c:when test="${product.odr_status == 2}">배송준비</c:when>
													<c:when test="${product.odr_status == 3}">배송중</c:when>
													<c:when test="${product.odr_status == 4}">배송완료</c:when>
												</c:choose>
											</td>
											<td class="col-md-1">
