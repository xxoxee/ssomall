...
<%-- 클릭 이벤트 메소드 --%>
<script type="text/javascript" src="/js/admin/board/notice.js"></script>

<%--메세지 처리 --%>
<script>
	if("${msg}" == "WRITE_NOTICE_SUCCESS"){
		alert("공지사항이 등록되었습니다.");
	}
...
<script>
	$(document).ready(function(){
		...
		$("#btn_modal_reply").on("click", function(){
			...
			$("#title_modal").val("");
			$("#content_modal").val("");
			$("#writer_id_modal").val("${sessionScope.admin.admin_id}");
		...
		$("#btn_search").on("click", function(){
			...
			if(term.val== null || term.val()=="") {
				...
				location.href= "/admin/board/notice/${pm.makeQuery(1)}&searchType=" + searchType.val()
							+ "&keyword=" + keyword.val();
			}
		});
		...
		$(".modal_close").on("click", function(){
			$("#modal").hide();
			location.href= "/admin/board/notice/${pm.makeSearch(pm.cri.page)}";
		});
		...
		$("#btn_modal_modify").on("click", function(){
			...
			var result = confirm("이대로 공지를 수정하시겠습니까?");
		...
		$("#btn_modal_delete").on("click", function(){
			...
			if(result){
				$.ajax({
					url:"/admin/board/deleteNotice/"+brd_num,
					headers: { 
	...
	<div class="wrapper">
		...
		<div class="content-wrapper">
			...
			<section class="content-header">
				<h1>
					공지사항 목록 <small>Notice List</small>
				</h1>
				<ol class="breadcrumb">
					<li><a href="#"><i class="fa fa-dashboard"></i> 게시판</a></li>
					<li class="active">공지사항 목록</li>
				</ol>
			</section>
							...
							<div style="margin-left:30px;">
								...
								<table class="search" style="display:inline-block;">
									<tr class="search" style="height:70px;" >
										<th rowspan="2" style="padding-right:50px; background-color:#EEEEEE;">기간(작성일)</th>
									...
									<tr></tr>
									<tr class="search">
										<th style="padding-right:50px; background-color:#EEEEEE;"> 검색 키워드 </th>
